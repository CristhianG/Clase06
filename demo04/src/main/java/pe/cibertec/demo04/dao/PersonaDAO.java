package pe.cibertec.demo04.dao;
...
public class PersonaDAO {
    ...
    public ArrayList<Persona> listPersona(String generoNombre) {
        Cursor cursor = DataBaseSingleton.getInstance().rawQuery("Select p.* from Persona p join Genero g on p.generoId = g.generoId where g.nombre like ?", new String[]{"%"+generoNombre+"%"});
    ...
    public boolean insertPersona(Persona persona) {
        ...
        cv.put("generoId", persona.getGeneroId());
    ...
    private Persona transformCursorToPersona(Cursor cursor) {
        ...
        persona.setGeneroId(cursor.getInt(cursor.getColumnIndex("generoId")));
